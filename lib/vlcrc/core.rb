...
module VLCRC
  ...
  class VLC
    ...
    # in the @socket instance variable, or nil if there is a problem connecting
    def connect
      begin
    ...
    # Get current position in the file (in seconds)
    def position() ask( "get_time" ).to_i end

    # Seek to a given time in the file (in seconds)
    def position=( time ) ask "seek #{time}", false end

    # Get the total length of the current media (in seconds)
    def length() ask( "get_length" ).to_i end
    ...
    # return the first line of the response unless the second argument is false
